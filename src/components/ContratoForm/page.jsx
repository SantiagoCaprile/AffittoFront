...
export default function ContratoForm({ contratoId = null }) {
	...
	useEffect(() => {
		...
		if (contratoId) {
			Contrato.buscarContrato(contratoId).then((data) => {
				...
				setValue("comision_celebracion", data.comision_celebracion);
				setValue("observaciones", data.observaciones);
				setLocatario(data.locatario);
				setGarantes(data.garantes);
			});
		}
	}, [contratoId, setValue]);
	...
	const onSubmit = async (data) => {
		...
		if (!contratoId && garantes.length === 0) {
		...
		if (!contratoId) {
			...
			Contrato.editarContrato(contratoId, data)
				.then((data) => {
					console.log(data);
					router.push(`/propiedades/${propiedadId}/${contratoId}`);
				})
				.catch((error) => {
					console.error("Error:", error);
				});
		...
		<div className="flex flex-1 justify-center items-center bg-[#E8EFFF] pt-4 gap-2">
			<div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 max-w-[700px] w-1/2">
				<h2 className="text-2xl font-bold mb-4">
					{contratoId ? "Editar" : "Crear"} contrato de alquiler
				</h2>
				<form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
					...
					>
						{envio.loading ? (
							...
							<p>Guardar cambios del contrato</p>
						) : (
							<p>Crear Contrato</p>
						)}
					</button>
			...
			{contratoId && (
				<div className="grid grid-flow-dense gap-2 w-1/3 bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
					<div className="flex flex-col gap-2">
						<h3 className=" text-xl font-bold">Locatario</h3>
						<p>{locatario.nombre_razon_social}</p>
					</div>
					<hr />
					<div className="flex flex-col gap-2">
						<h3 className=" text-xl font-bold">Garantes</h3>
						{garantes.map((garante) => (
							<p key={garante.id}>{garante.nombre_razon_social}</p>
						))}
					</div>
					<hr />
					<p className={styles.errors}>
